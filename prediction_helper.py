...
def handle_scaling(age, df):
    # Choose the appropriate scaler based on age
    if age <= 25:
        scaler_object = scaler_young
    else:
        scaler_object = scaler_rest

    # Error handling for missing keys in scaler_object
    if 'scaler' not in scaler_object:
        raise KeyError("The scaler_object is missing the 'scaler' key")

    # Define the default columns to scale if 'cols_to_scale' is missing
    cols_to_scale = scaler_object.get('cols_to_scale', ['age','number_of_dependants','income_level','income_lakhs','insurance_plan', 'genetical_risk'])

    # Retrieve the scaler
    scaler = scaler_object['scaler']

    # Temporary 'income_level' to match the scaler's expected input
    df['income_level'] = None  # Dummy column for compatibility with the scaler

    # Perform the scaling
    df[cols_to_scale] = scaler.transform(df[cols_to_scale])

    # Remove the temporary 'income_level' column after scaling
